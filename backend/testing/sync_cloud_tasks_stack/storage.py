...
The production storage helpers remain in use.  This module replaces only the
``google.cloud.storage.Client`` leaf before the backend imports it, keeping all
bucket/blob calls local to the gauntlet state directory.
"""
...
def configure_storage_dir(path: str | Path) -> Path:
    """Set up the local bucket root exactly once for this app process."""
    global _storage_dir
...
class LocalBlob:
    """Subset of the Google Cloud Storage blob API exercised by this stack."""
    ...
    def generate_signed_url(self, *_args: Any, **_kwargs: Any) -> str:
        # A provider stub consumes this opaque URL.  It deliberately contains
        # neither a bucket/object path nor a real network target.
...
class LocalStorageClient:
    """Drop-in lazy client instantiated by production storage helpers."""
